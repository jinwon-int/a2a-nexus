...
const docPath = join(repoRoot, 'docs', 'validation', 'team1-workerAlpha-residue-cleanup-go-nogo.md');

async function doc() {
  return readFile(docPath, 'utf8');
}

test('Team1/workerAlpha residue cleanup matrix binds the required trackers', async () => {
...
test('Team1/workerAlpha residue cleanup matrix covers residue cleanup GO/NO-GO gates', async () => {
  const content = await doc();
...
test('Team1/workerAlpha residue cleanup matrix remains fail-closed for production mutation', async () => {
  const content = await doc();
...
test('Team1/workerAlpha residue cleanup matrix preserves terminal evidence and artifact hygiene boundaries', async () => {
  const content = await doc();
...
test('Team1/workerAlpha residue cleanup matrix distinguishes legacy residue from current gaps', async () => {
  const content = await doc();

...
const docPath = join(repoRoot, 'docs', 'validation', 'team1-workerDelta-go-nogo-evidence-pack.md');
...
test('Team1/workerDelta GO/NO-GO evidence packet binds parent lane and run identifiers', async () => {
...
test('Team1/workerDelta GO/NO-GO evidence packet references predecessor packs', async () => {
  const content = await doc();

  for (const reference of [
    'team1-workerDelta-release-readiness-four-repo-matrix.md',
    'team1-workerDelta-release-blocker-proof-matrix.md',
  ]) {
    assert.match(content, new RegExp(escapeRegExp(reference)));
  }
});

test('Team1/workerDelta GO/NO-GO evidence packet covers all four repos', async () => {
  const content = await doc();
...
test('Team1/workerDelta GO/NO-GO evidence packet covers evidence dimensions', async () => {
  const content = await doc();
...
test('Team1/workerDelta GO/NO-GO evidence packet documents 12 final gates', async () => {
  const content = await doc();
...
test('Team1/workerDelta GO/NO-GO evidence packet references sibling lanes', async () => {
  const content = await doc();

  for (const lane of ['workerGamma', 'workerBeta', 'workerAlpha']) {
...
test('Team1/workerDelta GO/NO-GO evidence packet fails closed as NO-GO', async () => {
  const content = await doc();
...
test('Team1/workerDelta GO/NO-GO evidence packet preserves safety and non-actions', async () => {
  const content = await doc();
...
test('Team1/workerDelta GO/NO-GO evidence packet excludes runtime/bootstrap context', async () => {
  const content = await doc();
...
test('Team1/workerDelta GO/NO-GO evidence packet includes verification output', async () => {
  const content = await doc();
...
test('Team1/workerDelta GO/NO-GO evidence packet includes changed files section', async () => {
  const content = await doc();

  for (const changedFile of [
    'docs/validation/team1-workerDelta-go-nogo-evidence-pack.md',
    'scripts/archive/check-team1-workerDelta-go-nogo-evidence-pack.test.mjs',
  ]) {
    assert.match(content, new RegExp(escapeRegExp(changedFile)));
  }
});

test('Team1/workerDelta GO/NO-GO evidence packet includes approval-sensitive blockers', async () => {
  const content = await doc();

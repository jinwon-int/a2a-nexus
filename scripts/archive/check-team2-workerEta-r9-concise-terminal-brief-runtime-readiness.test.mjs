...
const docPath = join(
  ...
  'docs',
  'validation',
  'team2-workerEta-r9-concise-terminal-brief-runtime-readiness.md',
);
...
test('R9 no-live proof renders compact known-total titles for all seven children', async () => {
  ...
  assert.doesNotMatch(content, /\(\d+\/\?\)|A2A Terminal Brief 완료: workerEta\(7\/\?\)/);
});
...
test('R9 validation enforces parent-only notification ownership', async () => {
  ...
  for (const phrase of [
    ...
    'brokerBeta must relay a bounded projection back to `brokerAlpha`',

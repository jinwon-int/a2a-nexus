...
const docPath = join(
  ...
  'docs',
  'validation',
  'team2-workerEta-r11-libero-cross-team-risk-review.md',
);
...
test('R11 Terminal Brief aggregation and activation remain parent-owned and approval-gated', async () => {
  ...
  for (const phrase of [
    'Parent `brokerAlpha` aggregation counts terminal PR/Done/Block evidence only',
    'Team2/brokerBeta projections are bounded, idempotent',
    'local notifications stay disabled',
    'Any attempt to count Start/running/provider evidence as terminal',
    'let `brokerBeta` own or duplicate the parent notification',
  ...
  assert.doesNotMatch(content, /brokerBeta owns the parent notification|historical outbox replay is allowed/i);
});

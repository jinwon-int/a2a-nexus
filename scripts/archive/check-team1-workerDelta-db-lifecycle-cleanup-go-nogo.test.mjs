...
const docPath = join(repoRoot, 'docs', 'validation', 'team1-workerDelta-db-lifecycle-cleanup-go-nogo.md');

async function doc() {
  return readFile(docPath, 'utf8');
}

test('Team1/workerDelta DB lifecycle cleanup matrix binds the required trackers', async () => {
...
test('Team1/workerDelta DB lifecycle cleanup matrix covers lifecycle GO/NO-GO gates', async () => {
  const content = await doc();
...
test('Team1/workerDelta DB lifecycle cleanup matrix remains fail-closed for production mutation', async () => {
  const content = await doc();
...
test('Team1/workerDelta DB lifecycle cleanup matrix preserves terminal evidence and artifact hygiene boundaries', async () => {
  const content = await doc();

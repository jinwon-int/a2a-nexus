...
const docPath = join(repoRoot, 'docs', 'validation', 'team1-workerDelta-r27-canary-hardening-libero.md');
...
test('R27 canary hardening libero matrix covers required run metadata', async () => {
  ...
  assert.match(content, /a2a-r27-team1-terminal-brief-canary-hardening-20260516T121247Z-workerDelta-retry1/);
  assert.match(content, /#364/);
  assert.match(content, /workerDelta/);

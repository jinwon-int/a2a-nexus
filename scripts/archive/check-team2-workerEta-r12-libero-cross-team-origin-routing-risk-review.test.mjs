...
const docPath = join(
  ...
  'docs',
  'validation',
  'team2-workerEta-r12-libero-cross-team-origin-routing-risk-review.md',
);
...
test('R12 Team2 handoff metadata is directionally explicit', async () => {
  ...
  for (const phrase of [
    ...
    '`brokerAlpha`',
    '`brokerBeta`',
    'Handoff payload assumes brokerBeta origin for a brokerAlpha-commanded round',
...
test('R12 symmetric origin-routing is origin-based and not brokerAlpha-hardcoded', async () => {
  const content = await doc();

  for (const phrase of [
    'The guard must be origin-based, not brokerAlpha-hardcoded',
    'If `brokerAlpha` initiates/commands the parent round',
    'owned by `brokerAlpha`',
    'If `brokerBeta` initiates/commands a future parent round',
    'owned by `brokerBeta`',
    'Handoff/execution broker is not the Terminal Brief owner unless it is also the origin/parent broker',
    'brokerAlpha→brokerBeta',
    'brokerBeta→brokerAlpha',
    '30–60 second verification window',
  ]) {
    assert.match(content, new RegExp(escapeRegExp(phrase)));
  }

  assert.doesNotMatch(content, /brokerAlpha is always the parent Terminal Brief owner|brokerBeta can never own parent aggregation/i);
});
...
test('R12 risk matrix preserves origin ownership, handoff containment, and ACK separation', async () => {
  ...
  for (const forbiddenClaim of [
    ...
    /handoff broker owns brokerAlpha-origin parent/i,
    /future brokerBeta-origin proof is complete/i,
  ]) {
    assert.doesNotMatch(content, forbiddenClaim);
  }
});

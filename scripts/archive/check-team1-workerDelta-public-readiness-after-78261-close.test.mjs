...
const docPath = join(repoRoot, 'docs', 'validation', 'team1-workerDelta-public-readiness-after-78261-close.md');

async function doc() {
  return readFile(docPath, 'utf8');
}

test('Team1/workerDelta post-78261 matrix ties the lane to #75, #94, and #261', async () => {
...
test('Team1/workerDelta post-78261 matrix preserves provider accepted-send non-ACK boundary', async () => {
  const content = await doc();
...
test('Team1/workerDelta post-78261 matrix keeps all readiness gates fail-closed', async () => {
  const content = await doc();
...
test('Team1/workerDelta post-78261 matrix documents runtime hygiene and unsafe-action exclusions', async () => {
  const content = await doc();

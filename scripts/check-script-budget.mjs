...
export const BUDGETS = {
  scriptsMjs: 160, // +1 from 159: scripts/retrieval-manifest-contract.test.mjs — broker↔dispatch shared contract test for the signed web-retrieval snapshot gateway (#2017, #2019). // -2 from 161: the split-repo-local-demo conformance checker and its test retire with the demo. The demo walked a three-package split-repo story; one of those packages (openclaw-plugin-a2a) is gone and the split repos are provenance mirrors, so the topology it described no longer exists.
  ...
  brokerNpmScripts: 57, // +1 from 56: #1504 Phase 5 adds smoke:migration-rehearsal — the plan.md Phase 5 migration/rollback rehearsal entrypoint (scripts/migration-rehearsal-v1.mjs). It cannot fold into smoke:restart-recovery: a different stage of the same plan with its own exit gate (zero-divergence migration verification + boundary-crash rollback), and folding one plan stage into another's alias would hide which stage an operator ran. // +1 from 55: #2080 step 2 adds build:tests — the test-including compile (tsconfig.json) that the test manifest step-02 and coverage:baseline must run now that `build` is the runtime-only config (tsconfig.build.json). It cannot fold into `build`: production/Docker builds must not compile 291 test files. // +1 from 54: #1766 adds build:image, the verified image-build entrypoint (preflight -> docker compose build). Deliberate: it is a build entrypoint, not a per-round wrapper, and it cannot fold into `build` — `build` also runs inside the Docker build stage, where invoking docker again is impossible. -43 from 95: #1503 Wave 2 collapses the orchestration-intelligence (21) and rollout-preflight (24→22; rollout_guard and worker_signature_rollout_preflight stay direct as manifest-required gates) aliases into the orchestration/rollout dispatchers sharing scripts/lib/operator-dispatch.mjs; scan:public-readiness stays direct (CI parity requiredScript).
  brokerCoreModules: 181, // +1 from 180: #2078 C3 adds core/task-lineage-index.ts — the incremental lineage index whose structures mirror the pure builder's (createTaskLineageReadIndex) so one view function serves both; folding it into task-lineage-read.ts would put mutable write-path state inside the module documented as deliberately pure, and folding it into broker.ts would bury a 300-line domain beside the broker god-class. // +1 from 179: #B6 adds core/round-progress-tracker.js — the single shared round-progress counter for the two terminal surfaces (SSE task-event-stream and the operator-facing terminal-event-outbox). It exists precisely to delete a duplicated implementation whose two copies reported different `n/N` numerators for the same round; folding it into either surface would make one sibling surface import the other and re-couple the drift this change removes. Net core modules for the change are +1, not +2: the derived request schemas were folded into core/store-schemas.ts, their single source of truth. // +1 from 178: #1800 slice 5 adds core/wave-plan-dag-v2-mode.ts — a two-line rollout-mode resolver whose posture (off default, invalid env throws at startup) must match review-lineage-store's resolver one-for-one; folding it there would couple an independent contract's gate to another domain's module. // +1 from 177: #1863 C3 slice 1 adds core/broker-conversation-task-bridge.ts — the task-lifecycle bridge (result projection + input-required resume) is a distinct concern from the envelope domain and the cross-broker relay. // -38 from 213: #1665 retires the terminal-brief sidecar activation ceremony. Two weeks of instrumentation on both brokers recorded zero real invocations of the 37 sidecar routes, with a control probe re-established on each to prove the probe fires. terminal-brief-sidecar-integration-rehearsal stays — it is reached from the live finalizer workflow, not from the ceremony chain.

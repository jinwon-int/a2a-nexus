...
Parent: #1798 (alternative path), #1601. Status: **Phase 2 code complete (2026-08-16) — WS1 (mirrored opt-in flag), WS2 (hardened executor extension, baked into the piri runner image), WS3 (tool allowlist wiring), WS4 (prompt/brief/mode) and WS5 (evidence-return generalization + schema block) all implemented code+tests; wiring design in [`phase-2-wiring.md`](phase-2-wiring.md) (Phase 1: [`phase-1-mapping.md`](phase-1-mapping.md); Phase 0: [`phase-0-findings.md`](phase-0-findings.md)); Phase 3 (shadow → canary → widen) needs operator approval per step; implementation stays default-off and gated.**

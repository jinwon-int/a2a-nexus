...
> gate. 18 did until 2026-09-24, when #2257 B2 set `archived: true` on every
> `scripts/archive/` entry (verify with `node scripts/run-release-gate.mjs --list
> | grep -c scripts/archive/` → `0`). Step 2 is only complete when the entry is
> gone; moving the file alone is cosmetic.

...
| `a2a-docker-runner` | Public source repo with MIT root license, source-public marker, green CI, and package `private: false`. | Add package `license`, review/add `files` allowlist, run package tarball dry-run, decide image publication boundary, close or defer Go/Java evidence support (#343), and obtain explicit npm/image approval. |

...
|---|---|---|
| Tag gate | Creating or moving any tag, including semantic versions and new source-public markers. | Repo, tag name, target commit SHA, whether tag is annotated/signed/lightweight, and confirmation that no deploy/publish is bundled. The a2a-docker-runner release-gate.yml enforces this via a `release` environment with manual approval before any non-dry-run tag step (see [#485](https://github.com/jinwon-int/a2a-plane/issues/485), `packages/docker-runner/.github/workflows/release-gate.yml`). |
...
| `a2a-docker-runner` | Public source repo with MIT root license, source-public marker, green CI, and package `private: false`. | Package `license` and `files` allowlist added by [#485](https://github.com/jinwon-int/a2a-plane/issues/485). Release/tag workflow gated behind a `release` environment approval gate via `packages/docker-runner/.github/workflows/release-gate.yml`. Remaining: run package tarball dry-run, decide image publication boundary, close or defer Go/Java evidence support (#343), and obtain explicit npm/image approval. |

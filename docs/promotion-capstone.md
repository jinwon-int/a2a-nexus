...
The #1506 async-safety approval is now executable in `check:source-quality-floors`: a type-aware zero floor (`floating-promises=0`) covers production source in all five TypeScript workspace packages (`attestation`, `broker`, `docker-runner`, `nclex-evaluation`, and `policy-referee`). Tests, declarations, generated output, and archives are excluded. Promise-valued expression statements must be awaited, returned, assigned, or rejection-handled; an explicit `void` is reserved for reviewed fire-and-forget work. The manifest pins both the zero floor and the exact package scope so either count or scope drift fails closed. Package-CI parity analyzes only its selected package while the root gate checks the full workspace.

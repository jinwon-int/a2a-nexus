...
| Feedback/notification readiness | Public issue-form files and PR template route public-safe feedback; [public feedback intake evidence](public-feedback-intake.md) records that GitHub metadata still reports no templates and broker GitHub poller health is `not_started`. | Keep #1169 open until live issue chooser rendering and external issue/PR monitoring are verified or separately approved. | Live provider/Telegram sends, webhook/poller activation, notification canaries, or broad announcement. |

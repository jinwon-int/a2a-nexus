...
A2A Nexus은 OpenClaw 작업을 브로커와 워커로 안전하게 나누어 실행하고, `Done` / `Block` / PR 링크 같은 터미널 증거를 모으는 알파 단계 프로젝트입니다. 아직 프로덕션용이 아니며, 설계·문서·안전 경계에 대한 피드백과 사용 사례 제안을 환영합니다. 공개 전에는 보안/히스토리 스캔과 운영자 승인 게이트를 반드시 통과합니다.

...
| Agent payment dispute packet | User delegation, agent identity, payment/task scope, completion proof, and release decision are hash-bound and offline-verifiable for a source-only dispute artifact. | Card-network authorization, funds availability, legal identity, tax/regulatory compliance, final chargeback liability, payment rail execution, or PCI card-data handling. | [Agent Payment Dispute Packet v0](../contracts/a2a/agent-payment-dispute-packet.md) |
...
| Dispute packet | agent payment dispute packet fixture + offline verifier | A2A does not decide final network liability, prove legal identity, handle PCI card data, or execute payment rails. |

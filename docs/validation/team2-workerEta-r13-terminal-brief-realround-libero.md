# Team2/workerEta R13 compact Terminal Brief real-round libero validation
...
| `originBrokerId` | `brokerAlpha` | Missing, rewritten to the handoff/execution broker, or not equal to the commanding broker. |
...
| `parentBrokerId` / Terminal Brief owner | Must equal `originBrokerId`; `brokerAlpha` for this round | brokerBeta or any child execution broker owns, updates, duplicates, retracts, or ACKs the parent Terminal Brief. |
| `terminalEvidenceKind` | `PR`, `Done`, or `Block` only | Start/running/provider accepted-send evidence is counted as terminal closeout. |
| Compact title | `A2A Terminal Brief 완료: <worker>(n/7)`; this lane target is `A2A Terminal Brief 완료: workerEta(7/7)` | Details are moved into the title, the title omits the known total, or body/evidence is treated as ACK proof. |
...
| `crossBrokerHandoff.originBrokerId` | `brokerAlpha` | Handoff payload assumes brokerBeta origin for this brokerAlpha-commanded round. |
...
1. brokerAlpha initiated/commanded this R13 parent round, so brokerAlpha owns the parent Terminal Brief aggregation.
2. brokerBeta may execute Team2 handoff work and relay bounded child evidence, but brokerBeta must not own, duplicate, ACK, or close the brokerAlpha-origin parent Terminal Brief.
3. A future brokerBeta-origin parent round must aggregate to brokerBeta; the guard must remain origin-based and not brokerAlpha-hardcoded.
...
| --- | --- | --- | --- |
| Team1/workerGamma — [a2a-broker#608](https://github.com/jinwon-int/a2a-broker/issues/608) | Fail-closed dispatch guard implementation for missing parent metadata. | Dispatch expected title `A2A Terminal Brief 완료: workerGamma(1/7)` at [issuecomment-4446617399](https://github.com/jinwon-int/a2a-broker/issues/608#issuecomment-4446617399); Start marker at [issuecomment-4446632710](https://github.com/jinwon-int/a2a-broker/issues/608#issuecomment-4446632710). | `NO-GO / Waiting`; Start is not terminal evidence. |
| Team1/workerBeta — [openclaw-plugin-a2a#307](https://github.com/jinwon-int/openclaw-plugin-a2a/issues/307) | Plugin/runtime compact Terminal Brief verification with receipt/ACK boundary. | Dispatch expected title `A2A Terminal Brief 완료: workerBeta(2/7)` at [issuecomment-4446618127](https://github.com/jinwon-int/openclaw-plugin-a2a/issues/307#issuecomment-4446618127); Start marker at [issuecomment-4446626360](https://github.com/jinwon-int/openclaw-plugin-a2a/issues/307#issuecomment-4446626360). | `NO-GO / Waiting`; plugin receipt safety cannot be inferred from Start. |
| Team1/workerAlpha — [a2a-broker#609](https://github.com/jinwon-int/a2a-broker/issues/609) | Parent aggregation and ACK-boundary hardening. | Dispatch expected title `A2A Terminal Brief 완료: workerAlpha(3/7)` at [issuecomment-4446620137](https://github.com/jinwon-int/a2a-broker/issues/609#issuecomment-4446620137); Start marker at [issuecomment-4446630702](https://github.com/jinwon-int/a2a-broker/issues/609#issuecomment-4446630702). | `NO-GO / Waiting`; ACK-boundary hardening remains open until terminal evidence. |
| Team1/workerDelta — a2a-plane#305 (a2a-plane#305, internal tracker private) | Contracts/runbook for all-hands metadata and finalizer acceptance matrix. | Dispatch expected title `A2A Terminal Brief 완료: workerDelta(4/7)` at [issuecomment-4446622958](a2a-plane#305 (internal tracker, private)#issuecomment-4446622958); Start marker at [issuecomment-4446629898](a2a-plane#305 (internal tracker, private)#issuecomment-4446629898). | `NO-GO / Waiting`; contract/runbook proof remains open until terminal evidence. |
| Team2/workerEpsilon — [a2a-broker#610](https://github.com/jinwon-int/a2a-broker/issues/610) | brokerBeta handoff parity for brokerAlpha-origin parent ownership. | Dispatch expected title `A2A Terminal Brief 완료: workerEpsilon(5/7)` at [issuecomment-4446623706](https://github.com/jinwon-int/a2a-broker/issues/610#issuecomment-4446623706); Start marker at [issuecomment-4446632488](https://github.com/jinwon-int/a2a-broker/issues/610#issuecomment-4446632488). | `NO-GO / Waiting`; handoff parity cannot be counted from Start. |
| Team2/workerZeta — [a2a-docker-runner#251](https://github.com/jinwon-int/a2a-docker-runner/issues/251) | Runner evidence context and compact titles without artifact leaks. | Dispatch expected title `A2A Terminal Brief 완료: workerZeta(6/7)` at [issuecomment-4446624377](https://github.com/jinwon-int/a2a-docker-runner/issues/251#issuecomment-4446624377); Start markers at [issuecomment-4446629640](https://github.com/jinwon-int/a2a-docker-runner/issues/251#issuecomment-4446629640) and [issuecomment-4446639319](https://github.com/jinwon-int/a2a-docker-runner/issues/251#issuecomment-4446639319). | `NO-GO / Waiting`; runner leak guard and terminal evidence remain open. |
| Team2/workerEta — a2a-plane#306 (a2a-plane#306, internal tracker private) | This libero validation of compact titles, parent-only ownership, and guard aggregation evidence. | Dispatch expected title `A2A Terminal Brief 완료: workerEta(7/7)` at [issuecomment-4446625430](a2a-plane#306 (internal tracker, private)#issuecomment-4446625430); Start markers at [issuecomment-4446630442](a2a-plane#306 (internal tracker, private)#issuecomment-4446630442) and [issuecomment-4446638467](a2a-plane#306 (internal tracker, private)#issuecomment-4446638467). | Pass for validation shape only after this PR/Done evidence exists; aggregate remains `NO-GO / Waiting`. |
...
| Parent-only ownership | Parent aggregation is owned by `originBrokerId=brokerAlpha`; brokerBeta execution lanes cannot steal or duplicate parent ownership. | Open until Team2 broker/runner evidence proves handoff containment. | Handoff broker owns, duplicates, retracts, ACKs, or closes the brokerAlpha-origin parent Terminal Brief. |

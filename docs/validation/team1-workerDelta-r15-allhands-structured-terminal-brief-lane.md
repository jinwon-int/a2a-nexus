...
Parent round: `a2a-r15-allhands-structured-terminal-brief-20260514T065457Z-04-workerDelta`
...
| --- | --- | --- | --- | --- | --- |
| `parentRoundId` | Origin broker (`brokerAlpha`) | Yes — into handoff envelope | Parent broker, handoff broker, child broker | Yes | Missing, truncated, rewritten, or inconsistent across children |
| `originBrokerId` | Origin broker (`brokerAlpha`) | Yes — into handoff envelope | Parent broker, handoff broker, child broker | Yes | Missing, rewritten to handoff or execution broker, or not equal to the commanding broker |
| `parentBrokerId` | Origin broker (`brokerAlpha`) | Yes — into handoff envelope | Parent broker (owns notification dispatch) | Yes | Handoff or child broker assumes notification ownership; parent dispatches more than one notification per round |
| `handoffBrokerId` | Origin broker (`brokerAlpha`) — set per handoff child | Yes — into handoff envelope | Handoff broker, child broker of record | Yes — per child | Missing for cross-broker children; present and non-empty for direct children with handoff participants |
| `parentRoundTotal` | Origin broker (`brokerAlpha`) | Yes — into handoff envelope | Parent broker (title renderer) | Yes | Missing when known; rendered as unknown-total title despite known total |
| `parentRoundOrder` | Origin broker (`brokerAlpha`) — per child assignment | Yes — into handoff envelope | Child broker, parent broker | Yes — per child | Missing, duplicated, out of 1..7 range, or inconsistent with dispatched lane |
...
```
Origin/parent broker (brokerAlpha)
  ├── mints parentRoundId, originBrokerId=brokerAlpha, parentBrokerId=brokerAlpha, parentRoundTotal=7
...
Origin broker (brokerAlpha)
  ...
  ├── creates handoff envelope → handoffBrokerId=brokerBeta with metadata tuple
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | `brokerAlpha` | `brokerAlpha` | *(none — direct)* | `brokerAlpha` | v0 (origin=parent, direct) | Team1 lanes 1-3 |
| 2 | `brokerAlpha` | `brokerAlpha` | A | A | s"eoseo-origin handoff" | Team2 lane 5 |
| 3 | `brokerAlpha` | `brokerAlpha` | B | B | brokerAlpha-origin handoff | Team2 lane 6 |
| 4 | `brokerAlpha` | `brokerAlpha` | C | C | brokerAlpha-origin handoff | Team2 lane 7 (this all-hands lane) |

> *Broker aliases used in this round: `brokerAlpha` (origin/parent/Team1 broker), `brokerBeta` (Team2 handoff broker). Future rounds may extend with brokerBeta-origin symmetric pairs.*
...
| --- | --- | --- | --- |
| G1. Metadata propagation completeness | Every R15 child carries `parentRoundId`, `originBrokerId=brokerAlpha`, `parentBrokerId=brokerAlpha`, `parentRoundTotal=7`, and `parentRoundOrder`. Team2 handoff children carry `handoffBrokerId=brokerBeta`. | Missing, rewritten, or inconsistent metadata across children; handoff metadata cannot be joined to parent round. | Pass: parent-terminal-brief-aggregation.md contract v1 defines metadata lifecycle with symmetric support; R15 round enforces per the propagation matrix. |
...
| G5. Parent-only notification ownership | Parent broker `brokerAlpha` renders the aggregate Terminal Brief title for the R15 round. Handoff/execution brokers must not send their own parent round notification. | brokerBeta sends a brokerAlpha-origin parent notification; title ownership ambiguous or split across brokers. | Pass: parent-terminal-brief-aggregation.md sections "Parent-only notification ownership" and symmetric rules define ownership by `parentBrokerId`. |
| G6. Symmetric origin-broker routing | Origin-based routing is symmetric: brokerAlpha-origin rounds owned by brokerAlpha; future brokerBeta-origin rounds would be owned by brokerBeta. Metadata encodes direction explicitly. | Code or docs special-case brokerAlpha; brokerBeta owns a brokerAlpha-origin parent; or brokerAlpha owns a brokerBeta-origin parent. | Pass: parent-terminal-brief-aggregation.md v1 includes symmetric sections; R15 round uses brokerAlpha-origin with all handoffs directed back to brokerAlpha. |
...
| Concise title correctness | Per-child titles follow known-total format with dynamic denominator. R15 title is `A2A Terminal Brief 완료: workerDelta(4/7)`. | Contract defines format; target title documented and within constraints. | Title exceeds 80 chars, contains forbidden content, treats denominator as a global constant, or uses wrong assigned total/order. |
| Parent-only ownership | Parent Terminal Brief owner is `brokerAlpha` (origin broker). brokerBeta handoff/execution lane must not render its own parent notification. | Contract and symmetric rules define origin-based ownership. | brokerBeta renders brokerAlpha-origin parent notification; ownership ambiguous or split. |
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | `workerGamma` | Team1 | `brokerAlpha` (direct) | `brokerAlpha`/`brokerAlpha` | *(none)* | `A2A Terminal Brief 완료: workerGamma(1/7)` |
| 2 | `workerBeta` | Team1 | `brokerAlpha` (direct) | `brokerAlpha`/`brokerAlpha` | *(none)* | `A2A Terminal Brief 완료: workerBeta(2/7)` |
| 3 | `workerAlpha` | Team1 | `brokerAlpha` (direct) | `brokerAlpha`/`brokerAlpha` | *(none)* | `A2A Terminal Brief 완료: workerAlpha(3/7)` |
| 4 | `workerDelta` | Team1 | `brokerAlpha` (direct) | `brokerAlpha`/`brokerAlpha` | *(none)* | `A2A Terminal Brief 완료: workerDelta(4/7)` |
| 5 | `workerEpsilon` | Team2 | `brokerBeta` (handoff) | `brokerAlpha`/`brokerAlpha` | `brokerBeta` | `A2A Terminal Brief 완료: workerEpsilon(5/7)` |
| 6 | `workerZeta` | Team2 | `brokerBeta` (handoff) | `brokerAlpha`/`brokerAlpha` | `brokerBeta` | `A2A Terminal Brief 완료: workerZeta(6/7)` |
| 7 | `workerEta` | Team2 | `brokerBeta` (handoff) | `brokerAlpha`/`brokerAlpha` | `brokerBeta` | `A2A Terminal Brief 완료: workerEta(7/7)` |
...
| --- | --- | --- | --- |
| 1 (direct) | `brokerAlpha→brokerAlpha` | *(none)* → `brokerAlpha` | `A2A Terminal Brief 완료: workerGamma(1/7)` (lanes 1-4) |
| 2 (handoff) | `brokerAlpha→brokerAlpha` | `brokerBeta→brokerBeta` | `A2A Terminal Brief 완료: workerEpsilon(5/7)` (lane 5) |
| 3 (handoff) | `brokerAlpha→brokerAlpha` | `brokerBeta→brokerBeta` | `A2A Terminal Brief 완료: workerZeta(6/7)` (lane 6) |
| 4 (handoff) | `brokerAlpha→brokerAlpha` | `brokerBeta→brokerBeta` | `A2A Terminal Brief 완료: workerEta(7/7)` (lane 7) |
...
# Team1-workerDelta plane gates validation
...
Guard scan performed at snapshot time on branch `a2a-patch-20260514-065749-a2a-r15-allhands-structured-terminal-brief-20260514T065457Z-04-workerDelta`:

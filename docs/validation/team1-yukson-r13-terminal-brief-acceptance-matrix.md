# Team1/workerDelta R13 compact Terminal Brief acceptance matrix
...
| G6. Symmetric origin-broker routing | Origin-based routing: brokerAlpha-origin rounds owned by brokerAlpha; brokerBeta-origin rounds owned by brokerBeta (for future rounds). Metadata encodes direction explicitly. | Code or docs special-case brokerAlpha; brokerBeta owns a brokerAlpha-origin parent; or brokerAlpha owns a brokerBeta-origin parent. | Pass: parent-terminal-brief-aggregation.md v1 includes symmetric sections; R12 changes proved symmetric contract. |
...
**Decision: `GO_CANDIDATE / Needs finalizer closeout` for the acceptance matrix. This lane verifies the per-child Terminal Brief semantics for workerDelta(4/7): every child terminal transition must emit/send its own compact Terminal Brief to the origin/commanding broker. Parent/final summary closeout is separate from these per-child terminal emits.**

The R13 Team1/workerDelta acceptance matrix is documented and locally validated. Parent metadata propagation, compact title format, brokerAlpha-origin routing symmetry, and receipt/ACK boundary are verified from existing contracts, fixtures, and validation artifacts. The primary guard (a2a-broker#598) fail-closed condition is met: `parentRoundId`, `originBrokerId`, and known total metadata are present in the R13 dispatch.

For this lane, the compact per-child Terminal Brief title is `A2A Terminal Brief 완료: workerDelta(4/7)`. The `/7` denominator is the R13 broker-assigned child task total, not an absolute/global value. This per-child terminal emit is separate from any parent/final summary.

## R13 residual risk matrix

| Risk area | Required R13 proof | Current risk posture | Fail-closed condition |
| --- | --- | --- | --- |
| Parent metadata propagation | Every child includes `parentRoundId=a2a-r13-terminal-brief-realround-20260514T013556Z`, `originBrokerId=brokerAlpha`, `parentRoundTotal=7`, and order. Team2 handoff children include explicit `crossBrokerHandoff` tuple. | Verified via contract and dispatch definition; runtime enforcement depends on broker implementation. | Dispatch missing any required field, rewrites origin, or accepts partial metadata across lanes. |
| Compact title correctness | Per-child titles follow known-total format with a dynamic denominator sourced from broker parent-round assignment metadata. For R13 that assigned total is 7. | Contract defines format; target title `A2A Terminal Brief 완료: workerDelta(4/7)` is documented and within constraints. | Title exceeds 80 chars, contains forbidden content, treats denominator as a global constant, or uses wrong assigned total/order. |
| brokerAlpha-origin parent ownership | Parent Terminal Brief owner is `brokerAlpha` (origin broker). brokerBeta handoff/execution lane must not render its own parent notification. | Contract and symmetric rules define origin-based ownership. | brokerBeta renders brokerAlpha-origin parent notification; ownership ambiguous or split. |
...
| --- | --- | --- | --- | --- |
| 1 | `n/a` | Team1 | `brokerAlpha` | `A2A Terminal Brief 완료: n/a(1/7)` |
| 2 | `n/a` | Team1 | `brokerAlpha` | `A2A Terminal Brief 완료: n/a(2/7)` |
| 3 | `n/a` | Team1 | `brokerAlpha` | `A2A Terminal Brief 완료: n/a(3/7)` |
| 4 | `workerDelta` | Team1 | `brokerAlpha` | `A2A Terminal Brief 완료: workerDelta(4/7)` |
| 5 | `n/a` | Team2 | `brokerBeta` (projected) | `A2A Terminal Brief 완료: n/a(5/7)` |
| 6 | `n/a` | Team2 | `brokerBeta` (projected) | `A2A Terminal Brief 완료: n/a(6/7)` |
| 7 | `n/a` | Team2 | `brokerBeta` (projected) | `A2A Terminal Brief 완료: n/a(7/7)` |
...
Guard scan performed at snapshot time on branch `a2a-patch-20260514-013721-a2a-r13-terminal-brief-realround-20260514T013556Z-04-workerDelta`:

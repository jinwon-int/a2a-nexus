# R25 Team1 workerGamma operations-readiness gate for Team2 Terminal Brief → brokerAlpha production
...
This is the Team1/workerGamma operations-readiness gate for accepting Team2/brokerBeta Terminal Brief changes into brokerAlpha production. It defines the production safety gate, rollback criteria, default-off checks, and operator approval boundaries that Team1/brokerAlpha must verify before Team2 Terminal Brief feature code is activated in the brokerAlpha production broker and worker fleet.
...
The production safety gate defines the conditions under which Team2 Terminal Brief changes may be accepted into brokerAlpha production. It is a **Team1-operators-review** gate, not an automated gate — an operator must review the evidence and affirm the decision.

### G1.1 Preconditions for acceptance

| Condition | Verification evidence | Fail-closed condition |
|-----------|---------------------|-----------------------|
| Team2 Terminal Brief PR has passed its own review, CI, and conformance tests in the brokerBeta broker repo | PR link, CI run link, conformance test output (redacted) | PR is open, CI is red, or conformance fails |
...
| The brokerAlpha broker image/tag planned for deploy is pinned (not `latest`) and has a verifiable CI pass | Image tag and CI run URL | Image is unpinned or CI is untrusted |
| The brokerAlpha worker fleet (`workerGamma`, `workerEpsilon`, `workerBeta`) is patched to the commit that includes the Terminal Brief changes | Worker commit SHAs recorded and verified | Worker fleet version is not confirmed |
...
Rollback is the process of reverting the brokerAlpha production state to a pre-Terminal-Brief baseline. Rollback must be rehearsable and evidence-producing before live activation.
...
Before Terminal Brief can be activated in brokerAlpha production, an operator must successfully rehearse the rollback procedure in a non-production environment and post redacted evidence.
...
Terminal Brief must be **default-off** in brokerAlpha production. Activation requires explicit operator approval for a bounded window. These checks verify the default-off posture.
...
| Cross-broker Terminal Brief relay | brokerAlpha does not relay Terminal Brief to downstream brokers unless explicitly enabled | Relay config is absent or has `relayEnabled: false` |

### G3.2 Default-off verification checklist

Before accepting Team2 Terminal Brief changes into brokerAlpha production, verify every item:
...
|--------|-------------------|-----------------|
| Accept Team2 Terminal Brief PR into brokerAlpha production (merge decision) | ✅ Required | Operator comment on the lane issue or parent issue confirming precondition evidence is reviewed |
...
2. **Name the exact action being approved** — e.g., "I approve deploying broker image v1.2.3-terminal-brief-rc1 to the brokerAlpha production environment for a bounded 2-hour observation window starting at 2026-05-16T00:00Z."
...
| Team2 code changes only validated in brokerBeta, not in brokerAlpha | G1 requires brokerAlpha-specific CI and conformance pass | Not yet executed — Team2 PR is not yet available for this round | Terminal Brief accepted without brokerAlpha-side validation |
...
|--------|-----------------|------------------------|
| G1: Production safety gate | `contracts/a2a/terminal-semantics.md`, `contracts/a2a/task-lifecycle.md`, `contracts/a2a/r20-stability-gate.md` | `docs/release-gate.md`, `docs/validation/team1-workerGamma-r23-spec-first-acceptance-contract.md` |
| G2: Rollback criteria | `packages/broker/docs/production-stabilization-20260429.md`, `packages/broker/docs/terminal-brief-live-readiness-go-no-go-matrix.md` | `docs/validation/team1-workerDelta-terminal-brief-activation-libero.md` |
...
This lane publishes the ops-readiness gate definition and a `GO_CANDIDATE` decision. It defines what Team1/brokerAlpha must verify before accepting Team2 Terminal Brief changes into brokerAlpha production. It does not claim that Team2 code has been accepted, that production activation is GO, or that operator approval has been obtained. Production acceptance remains a later Team1 operator decision that references this gate definition.

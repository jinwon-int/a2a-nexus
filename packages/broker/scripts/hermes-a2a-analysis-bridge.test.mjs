...
test("Hermes A2A analysis bridge consumes sourceFiles as equivalent embedded evidence (#1270)", () => {
  const tempDir = mkdtempSync(join(tmpdir(), "hermes-a2a-bridge-sourcefiles-carrier-"));
  const fakeHermesPath = join(tempDir, "fake-hermes.mjs");
  const promptPath = join(tempDir, "prompt.txt");
  ...
  try {
    writeFileSync(fakeHermesPath, [
      ...
      "const prompt = args[args.indexOf('-q') + 1];",
      "writeFileSync(process.env.CAPTURE_PROMPT_PATH, prompt);",
      "if (!prompt.includes('CANARY_SOURCEFILES_MARKER')) throw new Error('sourceFiles marker missing from prompt');",
      "console.log(JSON.stringify({status:'done',summary:'sourceFiles carrier ok',findings:[],risks:[],recommendations:[],evidenceRefs:['CANARY-SOURCEFILES-ONLY.md']}));",
      "",
    ].join("\n"));
    chmodSync(fakeHermesPath, 0o755);
    ...
    const payload = {
      ...
      sourceFiles: [{ path: "CANARY-SOURCEFILES-ONLY.md", content: "CANARY_SOURCEFILES_MARKER\n" + "v".repeat(80) }],
      sourceProjectionPolicy: {
        requiredPaths: ["CANARY-SOURCEFILES-ONLY.md"],
        minProjectedBytesPerRequiredFile: 40,
      },
    };
    const message = [
      "V2 rollout sourceFiles-only carrier canary.",
      "Payload JSON:\n" + JSON.stringify(payload),
    ].join("\n\n");

...
} from "../dist/core/orchestration-intelligence-runtime-approval-decision-evidence.js";
...
async function main() {
  ...
  else console.log(renderOIRuntimeApprovalDecisionEvidenceMarkdown(packet));
}

main().catch((error) => {
  console.error("orchestration-intelligence-runtime-approval-decision-evidence: " + (error instanceof Error ? error.message : String(error)));
  process.exit(2);
});

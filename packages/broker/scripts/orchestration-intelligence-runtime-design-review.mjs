...
} from "../dist/core/orchestration-intelligence-runtime-design-review.js";
...
async function main() {
  ...
  else console.log(renderOIRuntimeDesignReviewMarkdown(packet));
}

main().catch((error) => {
  console.error("orchestration-intelligence-runtime-design-review: " + (error instanceof Error ? error.message : String(error)));
  process.exit(2);
});

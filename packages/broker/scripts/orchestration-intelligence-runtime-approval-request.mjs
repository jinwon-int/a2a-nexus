...
} from "../dist/core/orchestration-intelligence-runtime-approval-request.js";
...
async function main() {
  ...
  else console.log(renderOIRuntimeApprovalRequestMarkdown(packet));
}

main().catch((error) => {
  console.error("orchestration-intelligence-runtime-approval-request: " + (error instanceof Error ? error.message : String(error)));
  process.exit(2);
});

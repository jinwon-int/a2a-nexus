...
import { buildOIBrokerDispatchApprovalDecisionEvidencePacket } from "../dist/core/orchestration-intelligence-broker-dispatch-approval-decision-evidence.js";
import { buildOIWorkerSpawnApprovalRequestPacket } from "../dist/core/orchestration-intelligence-worker-spawn-approval-request.js";
import {
  buildOIWorkerSpawnApprovalDecisionEvidencePacket,
  renderOIWorkerSpawnApprovalDecisionEvidenceMarkdown,
} from "../dist/core/orchestration-intelligence-worker-spawn-approval-decision-evidence.js";
...
async function main() {
  ...
  const brokerDispatchApprovalDecisionEvidence =
    input.brokerDispatchApprovalDecisionEvidence
    ?? buildOIBrokerDispatchApprovalDecisionEvidencePacket({
      generatedAt: input.generatedAt,
      runId: input.brokerDispatchApprovalDecisionEvidenceRunId,
      recorder: input.recorder,
      expectedRepo: input.expectedRepo,
      expectedIssueNumber: input.expectedIssueNumber,
      notes: input.brokerDispatchApprovalDecisionEvidenceNotes,
      brokerDispatchApprovalRequest,
      decisionEvidence: input.decisionEvidence,
    });
  const workerSpawnApprovalRequest =
    input.workerSpawnApprovalRequest
    ?? buildOIWorkerSpawnApprovalRequestPacket({
      generatedAt: input.generatedAt,
      runId: input.workerSpawnApprovalRequestRunId,
      requester: input.requester,
      operator: input.operator,
      notes: input.workerSpawnApprovalRequestNotes,
      brokerDispatchApprovalDecisionEvidence,
      spawnEvidence: input.spawnEvidence,
    });
  const packet = buildOIWorkerSpawnApprovalDecisionEvidencePacket({
    ...
    decisionEvidence: input.decisionEvidenceForWorkerSpawnApproval,
  });
  if (argv.includes("--json")) console.log(JSON.stringify(packet, null, 2));
  else console.log(renderOIWorkerSpawnApprovalDecisionEvidenceMarkdown(packet));
}

main().catch((error) => {
  console.error(
    "orchestration-intelligence-worker-spawn-approval-decision-evidence: "

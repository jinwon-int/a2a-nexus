...
} from "../dist/core/orchestration-intelligence-runtime-readiness-gate.js";
...
async function main() {
  ...
  else console.log(renderOIRuntimeReadinessGateMarkdown(packet));
}

main().catch((error) => {
  console.error("orchestration-intelligence-runtime-readiness-gate: " + (error instanceof Error ? error.message : String(error)));
  process.exit(2);
});

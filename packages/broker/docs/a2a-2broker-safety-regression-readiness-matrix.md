...
Worker: `workerdelta` validation/matrix lane
...
| brokerbeta/Team2 onboarding runbook | #415 | PR/Done evidence for brokeralpha/brokerbeta conventions, stop/retarget/restart procedure, duplicate preflight requirement, rollback, and approval boundaries. | Blocks cutover until available and merged. |
...
Default decision is **NO-GO** until every linked lane has PR/Done evidence, the focused tests above pass in an integrated checkout, and duplicate preflight reports no online duplicate workerIds across brokeralpha/brokerbeta.

# Team1/workerdelta: #1032 A2AD Follow-up Review — #1083 Guardrail & #1086 CI-Triage

**⏱ Start marker: 2026-06-01T16:02:00Z**

**Worker:** workerdelta (Team1, lane 3/8)
**Parent issue:** [#1032 — Investigate residual periodic /livez latency stalls](https://github.com/jinwon-int/a2a-broker/issues/1032)
**PRs under review:**
- [#1083 — workerdelta: a2ad-1032-reused-socket-v2 guardrail](https://github.com/jinwon-int/a2a-broker/pull/1083)
- [#1086 — workerzeta: a2ad-1032-reused-socket-v2 keepAliveTimeout](https://github.com/jinwon-int/a2a-broker/pull/1086)
**Role:** Libero review from NO-GO perspective (workerdelta)
**Rule:** Source-only evidence + patch; no merge/deploy/live canary/DB mutation.

---

## 1. Review of #1083 — Libero Validation Matrix Guardrail (workerdelta)

### Summary of Changes

`#1083` (workerdelta, brokeralpha-ai) adds #1032-specific guardrails to the libero validation matrix:
...
- All data includes workerdelta-attribution dialectic comments
...
- Tests require workerdelta attribution in dialectic elements
...
### Libero/NO-GO Assessment from workerdelta

From my (workerdelta, synthesis-risk-libero) perspective as a libero reviewer:
...
- **No-live enforcement.** All new regression gates (L8–L11) have `noLiveOnly: true`. This is non-negotiable for a libero gate — workerdelta insists.
...
- **Rollback gate added.** L11 requires R1–R5 rollback proof. This is the escape hatch if keepAliveTimeout reduces stalls but causes regression elsewhere (e.g., increased connection memory). workerdelta considers this essential.

**⚠️ Issues to flag (none blocking, but must be tracked):**

1. **Verdict gap on #1032 closure.** The guardrails correctly require *evidence* before #1032 can close, but they do not set a target loopback metric. The libero-scheduling-gate-criteria.md calls for >1s rate <2% and >3s = 0. The closure criterion C6 mentions "5%" threshold but this should be tightened to the libero gate criteria standard. workerdelta recommends adding an explicit `thresholdMs` or `acceptableRate` field to closure criteria for clarity — but as a follow-up, not a blocker for this PR.

2. **No cross-check against the libero-scheduling-gate-criteria.md doc.** The libero-scheduling-gate-criteria.md already defines a gate matrix with K1–K5 (keep-open) and R1–R5 (rollback) conditions. The new L8–L11 gates should cross-reference those. workerdelta suggests a comment in the new data pointing to that doc for the definitive criteria interpretation. Filed as a non-blocking documentation gap — workerdelta can add in a follow-up.

3. **`connection_tracking_diagnostics` area is tested only structurally.** Test L10 validates that `totalConnections|activeConnections` patterns appear in rendered markdown, but does not assert that the actual `/schedz` endpoint emits them. This is acceptable for the matrix-level gate, but workerdelta notes that a subsequent CI-level integration test should validate the runtime connection counter. Not a blocker for #1083; acceptable sweep.

**Libero verdict on #1083 (workerdelta):** ✅ **PASS as guardrail — no pre-merge blocker.** The guardrails correctly extend the libero matrix, fail closed on missing #1032 evidence, and preserve no-live safety. The 3 minor gaps above are tracked for follow-up. **#1083 does NOT close #1032; it sets the acceptance criteria for closing.**

---

## 2. Review of #1086 — keepAliveTimeout Configurable (workerzeta)

### Summary of Changes

`#1086` (workerzeta, jinon86) makes the HTTP server `keepAliveTimeout` configurable:
...
The CI reports a **build failure** (1 error, completion in 35s). The build step is `tsc -p tsconfig.json`. Without CI log access, workerdelta diagnosed by analyzing the diff against the current code.

**Root cause (workerdelta analysis):** The test file (`server.test.ts`) uses `optServer.runtime.server.address()` to extract the port for the keep-alive agent test. The `runtime.server` is typed as `Server` from `node:http`. However, the `runtime` object returned by `startTestServer` uses:
...
### Decision: Repair, Supersede, or Hold (workerdelta opinion)

**workerdelta recommends: REPAIR.**
...
- The workerzeta patch is **directionally correct**: 62s default, configurable via option or env, with headersTimeout margined up to stay above keepAliveTimeout.
...
**Hold** would block #1032 from receiving this fix. workerdelta (libero) judges that holding this PR would require either:
...
- Or proving the 62s default causes a regression (memory pressure from idle sockets). Given broker worker heartbeats are bounded (~dozens, not thousands), workerdelta judges this risk as low.

**Verdict on #1086 (workerdelta):** ✅ **REPAIR** the TypeScript strict issue and merge alongside #1083. The CI fix is a 2-line change.

---

## 3. What Must Be Fixed Before #1032 Can Close

From the libero/NO-GO perspective, workerdelta identifies these preconditions for #1032 closure:
...
## 4. #1032 Close Verdict (workerdelta)
...
3. 🔄 Deploy both to the brokeralpha broker in a read-only, no-live gate
...
**Team1/workerdelta Libero:** workerdelta
**Date:** 2026-06-01
...
- ✅ Test 17 ("libero #1032 validation matrix includes workerdelta attribution") confirms workerdelta attribution in all dialectic elements

### Changed Files
| File | Change | Source PR |
|---|---|---|
| `src/core/libero-validation-matrix.ts` | #1032 guardrails: 3 new areas, C6-C9, T6-T9, L8-L11 | #1083 (workerdelta) |
| `src/core/libero-validation-matrix.test.ts` | 12+ new test cases for #1032 guardrails | #1083 (workerdelta) |
| `src/server.ts` | Configurable keepAliveTimeout + headersTimeout, 62s default | #1086 (workerzeta, repaired) |
| `src/server.test.ts` | keepAliveTimeout config test, stronger reuse assertions | #1086 (workerzeta, repaired) |
| `docs/workerdelta-1032-a2ad-followup-03-review.md` | This review analysis | This PR |

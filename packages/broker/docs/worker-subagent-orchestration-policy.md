...
- **Contained-subagent conductor (OpenClaw / Hermes / Codex in-container harness)** — consumes the bounded `A2A_CONTAINED_SUBAGENTS_*` budget (see `packages/docker-runner/docs/contained-subagent-conductor.md`). Codex remains explicit opt-in and creates task-scoped custom profiles: low-cost explorer/researcher roles use `gpt-5.6-luna`/`max`, upper implementer/verifier roles stay on `gpt-5.6-sol`, and the parent/finalizer model is unchanged. The `claude-code` runner does **not** consume this env even when the flag is set.

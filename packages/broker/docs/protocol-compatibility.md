...
| AgentCard trust / signing | AgentCard and worker capability cards are **unsigned**. Trust is enforced through transport auth (edge-secret + requester-id), not portable signed metadata. Card signing (JWS) and secure-passport `CallerContext` validation are explicitly deferred — see `docs/agent-card-trust-model.md`. | No key infrastructure exists. Adding it is a significant cross-cutting change. The current unsigned model is safe because all A2A consumers authenticate through edge-secret before making JSON-RPC calls. Worker capability cards already prevent private data exposure through `visibility` controls and secret scanning. | `src/a2a/protocol-compatibility.test.ts` asserts that `AgentCard.signature` and `AgentCard.signedExtensions` are `undefined` (intentionally unsigned). The trust-model fixture in `src/fixtures/a2a-protocol-compatibility.ts` records the deferred state. |

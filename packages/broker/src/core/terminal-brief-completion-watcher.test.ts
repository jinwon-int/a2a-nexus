...
test("buildTerminalBriefCompletionPacket prepares ready closeout candidate without treating provider receipt as ACK", () => {
  const packet = buildTerminalBriefCompletionPacket({
    ...
    events: [
      eventFor("workergamma", "succeeded", { progress: 1, total: 3, prUrl: "https://github.com/jinwon-int/a2a-broker/pull/701" }),
      eventFor("workerbeta", "succeeded", { progress: 2, total: 3, doneUrl: "https://github.com/jinwon-int/a2a-broker/issues/689#issuecomment-workerbeta" }),
      eventFor("workeralpha", "succeeded", { progress: 3, total: 3, prUrl: "https://github.com/jinwon-int/a2a-broker/pull/702" }),
    ],
  }, { now: NOW });
...
test("buildTerminalBriefCompletionPacket blocks when expected worker evidence is missing", () => {
  const packet = buildTerminalBriefCompletionPacket({
    parentRoundId: "round-689",
    expectedWorkers: ["workergamma", "workerbeta"],
    events: [
      eventFor("workergamma", "succeeded", { progress: 1, total: 2, prUrl: "https://github.com/jinwon-int/a2a-broker/pull/701" }),
    ],
  }, { now: NOW });

  assert.equal(packet.decision, "blocked");
  assert.deepEqual(packet.missingWorkers, ["workerbeta"]);
...
test("buildTerminalBriefCompletionPacket blocks succeeded lanes without PR or Done evidence", () => {
  const packet = buildTerminalBriefCompletionPacket({
    ...
    events: [
      eventFor("workergamma", "succeeded", { progress: 1, total: 1 }),
    ],
  }, { now: NOW });

  assert.equal(packet.decision, "blocked");
  assert.deepEqual(packet.missingEvidence, ["workergamma: succeeded lacks PR/Done/Block evidence"]);
  ...
  const packet = buildTerminalBriefCompletionPacket({
    ...
    events: [
      eventFor("workergamma", "succeeded", {
        taskId: "task-workergamma-a",
        progress: 1,
        total: 1,
        prUrl: "https://github.com/jinwon-int/a2a-broker/pull/701",
      }),
      eventFor("workergamma", "failed", {
        taskId: "task-workergamma-b",
        progress: 1,
        total: 1,
...
test("renderTerminalBriefCompletionPacketMarkdown includes safety and lane evidence", () => {
  const packet = buildTerminalBriefCompletionPacket({
    ...
    expectedWorkers: ["workergamma"],
    events: [
      eventFor("workergamma", "succeeded", { progress: 1, total: 1, doneUrl: "https://github.com/jinwon-int/a2a-broker/issues/689#issuecomment-done" }),
    ],
  }, { now: NOW });
  ...
  assert.match(markdown, /workergamma \(1\/1\)/);

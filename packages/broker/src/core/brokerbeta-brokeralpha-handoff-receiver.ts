/**
 * Default-off safety validation for durable brokerbeta → brokeralpha handoff receiver input.
 *
 ...
 * can use it before accepting a GitHub comment/manifest into the brokeralpha broker
 * inbox and before returning sanitized evidence to a parent issue.
 */

export const brokerbeta_brokeralpha_RECEIVER_STATUSES = [
  "accepted",
  "running",
  "pr-open",
  "done",
...
export type brokerbetabrokeralphaReceiverStatus = typeof brokerbeta_brokeralpha_RECEIVER_STATUSES[number];

export interface brokerbetabrokeralphaHandoffCandidate {
...
export interface brokerbetabrokeralphaHandoffReceiverOptions {
  brokerOfRecord?: string;
  requestedByBroker?: string;
  targetTeam?: string;
...
export interface brokerbetabrokeralphaHandoffValidationResult {
  ok: boolean;
  status: brokerbetabrokeralphaReceiverStatus | "rejected";
  reason?: string;
  sanitizedCandidate?: brokerbetabrokeralphaHandoffCandidate;
}

const DEFAULT_BROKER_OF_RECORD = "brokeralpha";
const DEFAULT_REQUESTED_BY_BROKER = "brokerbeta";
...
export function validatebrokerbetabrokeralphaHandoffCandidate(
  candidate: brokerbetabrokeralphaHandoffCandidate,
  opts: brokerbetabrokeralphaHandoffReceiverOptions,
): brokerbetabrokeralphaHandoffValidationResult {
...
export function parseA2AAssignComment(comment: string): brokerbetabrokeralphaHandoffCandidate | undefined {
...
function sanitizeCandidate(candidate: brokerbetabrokeralphaHandoffCandidate): brokerbetabrokeralphaHandoffCandidate {
  ...
  ) as brokerbetabrokeralphaHandoffCandidate;
}

function normalizeStatus(status: string | undefined): brokerbetabrokeralphaReceiverStatus | undefined {
  const value = status ?? "accepted";
  return brokerbeta_brokeralpha_RECEIVER_STATUSES.includes(value as brokerbetabrokeralphaReceiverStatus)
    ? value as brokerbetabrokeralphaReceiverStatus
    : undefined;
}

function reject(reason: string, sanitizedCandidate: brokerbetabrokeralphaHandoffCandidate): brokerbetabrokeralphaHandoffValidationResult {

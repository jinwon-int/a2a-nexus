...
function readyEvents(): TerminalTaskOutboxEvent[] {
  return [
    ...
    eventFor("workerbeta", "succeeded", {
      progress: 2,
      total: 3,
      doneUrl: "https://github.com/jinwon-int/a2a-broker/issues/690#issuecomment-workerbeta-done",
    }),
...
test("buildTerminalBriefFinalCountCloseoutCandidate emits candidate after final (N/N) and ready worker evidence", () => {
  const candidate = buildTerminalBriefFinalCountCloseoutCandidate({
    ...
    finalCountSignals: [
      { id: "brief-final", text: "Terminal Brief: workeralpha completed (3/3)", createdAt: NOW },
    ],
    events: readyEvents(),
  }, { now: NOW });
...
test("buildTerminalBriefFinalCountCloseoutCandidate blocks partial M/N and names missing workers", () => {
  const candidate = buildTerminalBriefFinalCountCloseoutCandidate({
    ...
    expectedWorkers: ["workergamma", "workerbeta", "workeralpha"],
    finalCountSignals: [
      { id: "brief-partial", text: "Terminal Brief: workerbeta completed (2/3)", createdAt: NOW },
    ],
    events: readyEvents().slice(0, 2),
  }, { now: NOW });

  assert.equal(candidate.decision, "blocked");
  assert.deepEqual(candidate.missingWorkers, ["workeralpha"]);
...
test("structured and terminal-event count signals are normalized", () => {
  ...
  assert.equal(signals.some((signal) => signal.source === "terminal-event" && signal.worker === "workergamma"), true);
});

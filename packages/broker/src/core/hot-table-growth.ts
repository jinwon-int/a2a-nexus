...
export interface HotTableGrowthTableProjection {
  ...
  /** Sum of serialized payload bytes from the hot table, when the store can provide it. */
  totalPayloadBytes?: number;
  /** Which metric was used to compute estimatedMemoryBytes. Present on projections generated by projectHotTableGrowth. */
  memoryEstimateBasis?: "totalPayloadBytes" | "maxPayloadBytes";
  ...
  const tables = Object.entries(current.tables).map(
    ([table, entry]): HotTableGrowthTableProjection => {
      ...
      const growthRate = priorCount !== null && priorCount > 0
        ...
        : null;
      const measuredPayloadBytes = typeof entry.totalPayloadBytes === "number"
        ? Math.max(0, entry.totalPayloadBytes)
        : undefined;
      const estimatedMemoryBytes = measuredPayloadBytes ?? entry.count * entry.maxPayloadBytes;
      const memoryEstimateBasis = measuredPayloadBytes !== undefined ? "totalPayloadBytes" : "maxPayloadBytes";
      ...
      return {
        ...
        ...(measuredPayloadBytes !== undefined ? { totalPayloadBytes: measuredPayloadBytes } : {}),
        memoryEstimateBasis,

...
export A2A_OPENCLAW_ALLOW_NPM_INSTALL_FALLBACK='1'
export A2A_DOCKER_RUNNER_MODEL_SOURCE='native'
A2A_OPENCLAW_DEFAULT_MODEL='openai-codex/gpt-5.6-sol'
...
if [ -z "${A2A_OPENCLAW_THINKING:-}" ]; then
  export A2A_OPENCLAW_THINKING='high'
else
...
rm -rf /root/.openclaw
mkdir -p /root/.openclaw/agents/'worker-alpha'/agent
...
copy_file_if_exists /run/secrets/openclaw-dir/agents/'worker-alpha'/agent/auth-profiles.json /root/.openclaw/agents/'worker-alpha'/agent/auth-profiles.json
copy_file_if_exists /run/secrets/openclaw-dir/agents/'worker-alpha'/agent/auth-state.json /root/.openclaw/agents/'worker-alpha'/agent/auth-state.json
copy_file_if_exists /run/secrets/openclaw-dir/agents/'worker-alpha'/agent/models.json /root/.openclaw/agents/'worker-alpha'/agent/models.json
...
Contained subagents:
- Do not spawn OpenClaw subagents for this task. This runner keeps subagent fanout disabled unless the host explicitly opts in.

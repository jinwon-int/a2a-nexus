...
export A2A_OPENCLAW_ALLOW_NPM_INSTALL_FALLBACK='0'
export A2A_DOCKER_RUNNER_MODEL_SOURCE='legacy'
A2A_OPENCLAW_DEFAULT_MODEL='openai-codex/gpt-5.5'
...
copy_file_if_exists /run/secrets/openclaw-dir/agents/'main'/agent/auth-profiles.json /root/.openclaw/agents/'main'/agent/auth-profiles.json
copy_file_if_exists /run/secrets/openclaw-dir/agents/'main'/agent/auth-state.json /root/.openclaw/agents/'main'/agent/auth-state.json
copy_file_if_exists /run/secrets/openclaw-dir/agents/'main'/agent/models.json /root/.openclaw/agents/'main'/agent/models.json
...
printf 'contained_subagents_max=3\n' | tee -a /work/artifacts/summary.txt
...
Contained subagents:
- You may spawn up to 3 OpenClaw subagent(s) inside this same Docker task boundary when the assignment matches these reasons: context_heavy, broad_source_inspection, validation_split.
...
- Bound each helper evidence summary to 12000 bytes or less, redact secrets/private paths/session data, and do not include raw transcripts in repository files or comments.

...
A2A_CLAUDE_DEFAULT_MODEL=__A2A_PROFILE_defaultModel__
...
if [ -z "${A2A_CLAUDE_TIMEOUT_SEC:-}" ]; then
  export A2A_CLAUDE_TIMEOUT_SEC=__A2A_PROFILE_defaultTimeout__
else
  export A2A_CLAUDE_TIMEOUT_SEC
fi
if [ -z "${A2A_CLAUDE_CODE_TIMEOUT_SEC:-}" ]; then
  export A2A_CLAUDE_CODE_TIMEOUT_SEC=__A2A_PROFILE_codeTimeout__
else
...
A2A_CLAUDE_PATCH_BRIDGE=__A2A_PROFILE_bridgePath__
...
__A2A_PROFILE_turnBudgetExports__

...
function buildDiffHygieneBlock(task: RunnerTask, baseBranch: string): string {
  ...
  return [
    ...
    `# One node invocation converts every NUL-separated newline list to a JSON array line.`,
    `{ read -r CHANGED_PATHS_JSON; read -r BLOCKED_PATHS_JSON; read -r LOCKFILE_CHANGES_JSON;${scopeEnabled ? ` read -r SCOPE_DECLARED_JSON; read -r SCOPE_OUTSIDE_JSON;` : ``} } <<< "$(printf '%s\\0' "$CHANGED_PATHS" "$BLOCKED_PATHS" "$LOCKFILE_CHANGES"${scopeEnabled ? ` ${shellSingleQuote(normalizedDeclaredScopePaths.join("\n"))} "$SCOPE_OUTSIDE"` : ``} | node -e 'const fs=require("fs"); for (const part of fs.readFileSync(0,"utf8").split("\\0").slice(0,-1)) console.log(JSON.stringify(part.split(/\\r?\\n/).filter(Boolean)))')"`,
    ...(scopeEnabled ? [

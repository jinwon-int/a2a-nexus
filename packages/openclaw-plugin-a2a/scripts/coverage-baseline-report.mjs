 ...
 * parseable number (e.g. this package runs its tests from `.ts` rather than a
 * built `dist/*.test.js`), the report still succeeds with coveragePercent = null.
 *
...
function measureCoverageBestEffort(distTestFiles) {
  if (!distTestFiles.length) {
    return { coveragePercent: null, note: 'no built dist/*.test.js found (package runs tests from .ts); classification only' };
  }
  try {
    const res = spawnSync(
      process.execPath,

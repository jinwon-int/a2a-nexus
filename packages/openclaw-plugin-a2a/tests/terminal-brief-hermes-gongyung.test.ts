...
function envelope() {
  return {
    ...
    version: 1,
    id: "terminal-brief-hermes-gongyung-test",
    dedupeKey: "terminal-brief-hermes-gongyung-test",
    type: "success",
    severity: "info",
    deliveryOwner: "openclaw.plugin-notifier",
    deliveryTarget: "operator-main-session",
    title: "A2A Terminal Brief 완료: gongyung(1/1)",
    text: "A2A Terminal Brief 완료: gongyung(1/1)\nsummary",
    evidence: {
      schema: "a2a.operator.notification.evidence",
      version: 1,
      taskId: "terminal-brief-hermes-gongyung-test",
      worker: "gongyung",
      status: "succeeded",
    },
    taskId: "terminal-brief-hermes-gongyung-test",
...
function minimalEnvelope() {
  return {
    kind: "a2a.operator.notification",
    version: 1,
    id: "terminal-brief-hermes-gongyung-minimal-test",
    dedupeKey: "terminal-brief-hermes-gongyung-minimal-test",
    type: "success",
    severity: "info",
    deliveryOwner: "openclaw.plugin-notifier",
    deliveryTarget: "operator-main-session",
    ...
    taskId: "terminal-brief-hermes-gongyung-minimal-test",
    workerId: "gongyung",
    status: "succeeded",
  };
}
...
function runAdapter(args: string[] = [], input = envelope()) {
  const child = spawnSync(process.execPath, [
    "dist/terminal-brief-hermes-gongyung.js",
    ...args,
  ], {
    cwd: process.cwd(),
...
describe("Terminal Brief Hermes/Gongyung adapter skeleton", () => {
  it("defaults to dry-run/spool-only receipt without terminal ACK", () => {
    ...
    assert.equal(result.decision.receiptId, "hermes-gongyung:gongyung:terminal-brief-hermes-gongyung-test");
  });

  it("spools a public-safe record without treating spool as receipt", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "terminal-brief-hermes-gongyung-"));
    const spoolFile = join(tempDir, "spool.jsonl");
    try {
      const result = runAdapter(["--spool-file", spoolFile, "--operator", "gongyung"]);
      const record = JSON.parse(readFileSync(spoolFile, "utf8").trim());

      assert.equal(result.status, 0, result.stderr);
      assert.equal(result.decision.ackTerminalEvent, false);
      assert.equal(result.decision.terminalReceiptStatus, "produced");
      assert.equal(record.schema, "a2a.terminalBrief.hermesGongyungAdapter.spool.v1");
      assert.equal(record.operator, "gongyung");
      assert.equal(record.taskId, "terminal-brief-hermes-gongyung-test");
  ...
  it("spools a minimal workerId envelope without evidence", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "terminal-brief-hermes-gongyung-minimal-"));
    const spoolFile = join(tempDir, "spool.jsonl");
    try {
      const result = runAdapter(["--spool-file", spoolFile, "--operator", "gongyung"], minimalEnvelope());
      const record = JSON.parse(readFileSync(spoolFile, "utf8").trim());

      assert.equal(result.status, 0, result.stderr);
      assert.equal(result.decision.ackTerminalEvent, false);
      assert.equal(record.taskId, "terminal-brief-hermes-gongyung-minimal-test");
      assert.equal(record.worker, "gongyung");
  ...
  it("allows explicit manual operator receipt projection", () => {
    const result = runAdapter(["--manual-receipt-id", "manual:gongyung:1"]);

    assert.equal(result.status, 0, result.stderr);
    assert.deepEqual(result.decision, {
      ackTerminalEvent: true,
      confirmationSource: "manual_operator_receipt",
      receiptId: "manual:gongyung:1",
      reason: "Hermes/Gongyung adapter received manual operator receipt",
    });
  });

  it("allows explicit current-session visible receipt projection", () => {
    const result = runAdapter(["--visible-receipt-id", "hermes:gongyung:visible:1"]);

    assert.equal(result.status, 0, result.stderr);
    assert.deepEqual(result.decision, {
      ackTerminalEvent: true,
      confirmationSource: "current_session_visible",
      receiptId: "hermes:gongyung:visible:1",
      reason: "Hermes/Gongyung adapter received current-session-visible confirmation",
    });
  });

  it("ACKs current-session-visible receipt evidence matched by envelope id", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "terminal-brief-hermes-gongyung-receipt-"));
    try {
      const evidenceFile = writeReceiptEvidence(tempDir, [{
        schema: "a2a.terminalBrief.hermesGongyung.receipt.v1",
        operator: "gongyung",
        envelopeId: "terminal-brief-hermes-gongyung-test",
        confirmationSource: "current_session_visible",
        status: "visible",
        receiptId: "hermes:gongyung:visible:1",
      ...
      assert.deepEqual(result.decision, {
        ...
        confirmationSource: "current_session_visible",
        receiptId: "hermes:gongyung:visible:1",
        reason: "Hermes/Gongyung receipt evidence confirmed current_session_visible",
      });
    } finally {
      rmSync(tempDir, { recursive: true, force: true });
    }
  ...
  it("ACKs manual operator receipt evidence matched by task id", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "terminal-brief-hermes-gongyung-manual-"));
    try {
      const evidenceFile = writeReceiptEvidence(tempDir, [{
        schema: "a2a.terminalBrief.hermesGongyung.receipt.v1",
        operator: "gongyung",
      ...
      assert.deepEqual(result.decision, {
        ...
        confirmationSource: "manual_operator_receipt",
        receiptId: "manual:gongyung:1",
        reason: "Hermes/Gongyung receipt evidence confirmed manual_operator_receipt",
      });
    } finally {
      rmSync(tempDir, { recursive: true, force: true });
    }
  ...
  it("does not ACK provider-only receipt evidence", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "terminal-brief-hermes-gongyung-provider-"));
    try {
      const evidenceFile = writeReceiptEvidence(tempDir, [{
        schema: "a2a.terminalBrief.hermesGongyung.receipt.v1",
        operator: "gongyung",
  ...
  it("marks matching stale receipt evidence as stale without ACK", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "terminal-brief-hermes-gongyung-stale-"));
    try {
      const evidenceFile = writeReceiptEvidence(tempDir, [{
        schema: "a2a.terminalBrief.hermesGongyung.receipt.v1",
        operator: "gongyung",
        ...
        confirmationSource: "current_session_visible",
        status: "visible",
        receiptId: "hermes:gongyung:visible:old",
        observedAt: staleIso(),
      }]);
  ...
  it("rejects receipt evidence for a different operator or envelope", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "terminal-brief-hermes-gongyung-mismatch-"));
    try {
      const evidenceFile = writeReceiptEvidence(tempDir, [{
        schema: "a2a.terminalBrief.hermesGongyung.receipt.v1",
        operator: "other",
        ...
        confirmationSource: "current_session_visible",
        status: "visible",
        receiptId: "hermes:gongyung:visible:other",
        observedAt: recentIso(),
      }]);

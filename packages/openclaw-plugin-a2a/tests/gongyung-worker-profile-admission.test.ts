...
function gongyungAdmit(
  ...
  if (task.mode) {
    ...
    let modeResult: AdmissionDecision;
    ...
    // Mode is allowed; check proof level compatibility
  ...
  // If mode isn't set but capabilities are, check them
...
// ════════════════════════════════════════════════════════════════
// §11  Fail-closed defaults: unknown modes/intents
// ════════════════════════════════════════════════════════════════
...
describe("Gongyung profile — fail-closed defaults", () => {
  ...
  it("completely unknown intent with no mode defaults to NO-GO (capabilities not matched)", () => {
    // With no mode set and no matching capability rejection, the default admit applies.
    // But if the intent is entirely unknown (not in allow or reject sets), it should admit
    // with lightweight proof per the default behavior. This is intentional: the known-safety
    // intent list is a guidance, not a hard block — the mode guard and capability guard
    // provide the hard boundaries.
    const result = gongyungAdmit({ id: "t2", intent: "something_new_unlisted" });
    assert.ok(result.ok); // Unknown intent with no mode/capability guard hit → default admit
    // This is by spec design: mode guard is the primary admission gate.
  });

  it("unknown intent with reject-capability-related payload defaults to NO-GO", () => {
    const result = gongyungAdmit({
      id: "t3",

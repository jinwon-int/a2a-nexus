...
function isIntentRejected(intent?: string): boolean {
  // Missing intent → NO-GO (fail-closed)
  ...
  if (task.intent && !isIntentAdmissible(task.intent)) {
    return {
      ...
      noGoSignal: {
        type: "team_assignment_no_go",
        nodeId: "gongyung",
        reason: `intent_not_allowed: ${task.intent} (not in admissible set: analyze, review, clarify, observe, check_readiness, cross_check)`,
        timestampMs: now(),
        observeOnly: true,
        handoffTargets,
      },

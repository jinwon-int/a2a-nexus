...
type HermesGongyungAdapterOptions = {
  operator: string;
  spoolFile?: string;
...
type HermesGongyungSpoolRecord = {
...
const DEFAULT_OPERATOR = "gongyung";
const DEFAULT_MAX_TEXT_CHARS = 4000;
const DEFAULT_RECEIPT_MAX_AGE_MS = 10 * 60 * 1000;
const RECEIPT_EVIDENCE_SCHEMA = "a2a.terminalBrief.hermesGongyung.receipt.v1";

async function main(argv: readonly string[] = process.argv.slice(2)): Promise<void> {
  const options = parseArgs(argv);
  const envelope = readEnvelopeFromStdin();
  const decision = runHermesGongyungAdapter(envelope, options);
  process.stdout.write(JSON.stringify(decision, null, 2) + "\n");
}

export function runHermesGongyungAdapter(
  envelope: A2AOperatorTerminalNotificationEnvelope,
  options: HermesGongyungAdapterOptions,
): A2ATerminalBriefDeliveryAdapterReceiptDecision {
  ...
  if (options.visibleReceiptId) {
    return {
      ...
      reason: options.reason ?? "Hermes/Gongyung adapter received current-session-visible confirmation",
    };
  }
  ...
  if (options.manualReceiptId) {
    return {
      ...
      reason: options.reason ?? "Hermes/Gongyung adapter received manual operator receipt",
    };
  }
  ...
  return {
    ...
    reason: options.reason ?? "Hermes/Gongyung adapter skeleton dry-run/spool only; operator-visible receipt is not confirmed",
    receiptId: "hermes-gongyung:" + options.operator + ":" + envelope.id,
  };
}
...
function parseArgs(argv: readonly string[]): HermesGongyungAdapterOptions {
  const options: HermesGongyungAdapterOptions = {
...
function readEnvelopeFromStdin(): A2AOperatorTerminalNotificationEnvelope {
  ...
  if (!text) {
    throw new Error("Hermes/Gongyung adapter requires a Terminal Brief envelope JSON on stdin");
  }
...
function validateEnvelope(envelope: A2AOperatorTerminalNotificationEnvelope): void {
  if (envelope?.kind !== "a2a.operator.notification") {
    throw new Error("Hermes/Gongyung adapter received an invalid envelope kind");
  }
  if (!normalizeOptionalString(envelope.id) || !normalizeOptionalString(envelope.dedupeKey)) {
    throw new Error("Hermes/Gongyung adapter requires envelope id and dedupeKey");
  }
}

function appendSpoolRecord(filePath: string, record: HermesGongyungSpoolRecord): void {
...
function buildReceiptDecisionFromEvidenceFile(
  ...
  if (records.length === 0) {
    return rejectedReceiptDecision("failed", "Hermes/Gongyung receipt evidence file had no records");
  }
  ...
  for (const record of records.slice().reverse()) {
    ...
    if (validation.ok) {
      return {
        ...
        reason: options.reason ?? "Hermes/Gongyung receipt evidence confirmed " + validation.confirmationSource,
      };
    }
  ...
  return rejectedReceiptDecision(
    strongestStatus,
    "Hermes/Gongyung receipt evidence rejected: " + (reasons[0] ?? "no matching record"),
  );
}
...
function readReceiptEvidenceRecords(filePath: string): { ok: true; records: unknown[] } | { ok: false; reason: string } {
  try {
    ...
    return {
      ok: false,
      reason: "Hermes/Gongyung receipt evidence file could not be read or parsed: " + (
